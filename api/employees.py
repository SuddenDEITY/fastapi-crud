...
async def read_employees(db: AsyncSession = Depends(get_db), skip: int = 0, limit: int = 100):
    employees = await get_employees(db=db, skip=skip, limit=limit)
    return employees


@router.post("/employees", response_model=Employee, status_code=201)
async def create_new_employee(employee: EmployeeCreate, db: AsyncSession = Depends(get_db)):
    db_employee = await get_employee_by_phone_number(db=db, phone_number=employee.phone_number)
    if db_employee:
...
async def full_update_employee(employee_id: int, employee: EmployeeCreate, db: AsyncSession = Depends(get_db)):
    db_employee = await put_update_employee(db=db, employee_id=employee_id, employee=employee) 
    return db_employee


@router.patch("/employees/{employee_id}", response_model=Employee, status_code=200)
async def partial_update_employee(employee_id: int, employee: EmployeeUpdate, db: AsyncSession = Depends(get_db)):
    db_employee = await patch_update_employee(db=db, employee_id=employee_id, employee=employee) 
    return db_employee
